...
// https://www.youtube.com/watch?v=N1pfsAjMSgg
let juego;
let img;
...
function preload() {
  ...
  imgInstrucciones = loadImage("data/imagenes/instrucciones.jpg");
...
class Juego {
  constructor() {
    this.esferas = [];
    this.cant = 7;
    this.puntos = new Puntos(); // Instancia clase Puntos
    this.vidas = new Vidas(); // Instancia clase Vidas
  ...
  setup() {
    for (let i = 0; i < this.cant; i++) {
      this.esferas[i] = new esferas(); // Inician las basuras
    }
    sonido.loop();
  }
    ...
    for (let i = 0; i < this.cant; i++) {
      if (this.esferas[i].evaluaColision(mouseX, mouseYFixed)) {
        if (this.esferas[i].tipo == 0) {
          this.puntos.incrementarPunto();
        } else {
          this.vidas.reducirVida(); 
